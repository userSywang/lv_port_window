...
// 单元格大小将在run方法中动态计算
#define CELL_SIZE(grid_size)	((short int)((grid_size - 5 * GRID_PAD) / 4))
...
Game2048::Game2048(uint16_t width, uint16_t height):
...
bool Game2048::run(void)
{
    ...
    // 判断是横屏还是竖屏
    bool is_landscape = _width > _height;
    ESP_LOGI(TAG, "Screen size: %dx%d, %s mode", _width, _height, is_landscape ? "landscape" : "portrait");

    // 计算棋盘大小和位置
    int grid_size, board_width, board_height;
    
    if (is_landscape) {
        // 横屏模式：棋盘在右侧，控制面板在左侧
        // 左侧占据1/3，右侧占据2/3
        board_width = _width / 3;
        board_height = _height;
        
        // 右侧区域宽度
        int right_width = _width - board_width;
        
        // 计算棋盘大小，取右侧区域宽度和屏幕高度的较小值，确保棋盘完全显示
        grid_size = (right_width < _height) ? right_width : _height;
        
        // 确保棋盘大小是4的倍数加上5倍的GRID_PAD，这样单元格大小计算会更精确
        grid_size = ((grid_size - 5 * GRID_PAD) / 4) * 4 + 5 * GRID_PAD;
    } else {
        // 竖屏模式：棋盘在下方，控制面板在上方
        grid_size = _width;
        board_width = _width;
        board_height = _height - grid_size;
    }

    /* Setup board */
    lv_obj_t *board = lv_obj_create(lv_scr_act());
    // Size
    lv_obj_set_size(board, board_width, board_height);
    // Position
    if (is_landscape) {
        lv_obj_align(board, LV_ALIGN_LEFT_MID, 0, 0);
    } else {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        lv_obj_align_to(_emoji_imgs[i], title, LV_ALIGN_OUT_BOTTOM_MID, 0, 10); // 增加一点垂直间距
        lv_obj_add_flag(_emoji_imgs[i], LV_OBJ_FLAG_HIDDEN);
    }

    lv_obj_t *cur = lv_obj_create(board);  // 创建在board上而不是屏幕上
    // Size
    lv_obj_set_size(cur, SCORE_WIDTH, SCORE_HEIGHT);
    // Position
    if (is_landscape) {
        // 在横屏模式下，将SCORE控件放在表情图标下方，留出足够空间
        lv_obj_align(cur, LV_ALIGN_TOP_MID, 0, 250);
    } else {
        // 在竖屏模式下，保持原来的位置
        lv_obj_align(cur, LV_ALIGN_TOP_MID, 0, 150);
    }
    // Shape
    ...
    lv_obj_t *best = lv_obj_create(board);  // 创建在board上而不是屏幕上
    // Size
    lv_obj_set_size(best, SCORE_WIDTH, SCORE_HEIGHT);
    // Position
    if (is_landscape) {
        lv_obj_align(best, LV_ALIGN_TOP_MID, 0, 20);
    } else {
        lv_obj_align(best, LV_ALIGN_TOP_LEFT, 30, 20);
    }
    // Shape
    ...
    /* Setup grid */
    // 计算单元格大小
    _cell_size = CELL_SIZE(grid_size);
    static lv_coord_t col_dsc[5]; // 需要动态设置
    static lv_coord_t row_dsc[5]; // 需要动态设置
    
    // 设置列和行的大小
    for (int i = 0; i < 4; i++) {
        col_dsc[i] = _cell_size;
        row_dsc[i] = _cell_size;
    }
    ...
    // Size
    lv_obj_set_size(background_grid, grid_size, grid_size);
    // Position
    if (is_landscape) {
        // 在右侧区域居中显示
        int right_area_center_x = board_width + (_width - board_width) / 2;
        lv_obj_set_pos(background_grid, 
                       right_area_center_x - grid_size / 2, 
                       (_height - grid_size) / 2);
    } else {
    ...
    // Size
    lv_obj_set_size(_foreground_grid, grid_size, grid_size);
    // Position
    if (is_landscape) {
        // 在右侧区域居中显示，与背景网格保持一致
        int right_area_center_x = board_width + (_width - board_width) / 2;
        lv_obj_set_pos(_foreground_grid, 
                       right_area_center_x - grid_size / 2, 
                       (_height - grid_size) / 2);
    } else {
    ...
    if (is_landscape) {
        // 在左侧区域底部居中显示，位于新游戏按钮上方
        lv_obj_align(_emoji_label, LV_ALIGN_BOTTOM_MID, 0, -100);
    } else {
        lv_obj_align(_emoji_label, LV_ALIGN_BOTTOM_LEFT, 10, -40);
    }
    lv_label_set_text(_emoji_label, "Welcome to play!"); // 修正拼写错误
...
void Game2048::generateForegroundCell(void)
{
    ...
    // Size
    lv_obj_set_size(cell, _cell_size, _cell_size);
    // Position
    lv_obj_set_pos(
        cell,
